    ...
    return (
      <div className="h-full flex items-center justify-center bg-background p-6">
        <div className="max-w-md text-center">
          ...
          <h3 className="text-xl font-bold text-white mb-3">전체 요약을 생성해보세요</h3>
          ...
          <div className="mb-4 flex items-center justify-center gap-2">
            ...
            />
            <label htmlFor="mock-emphasis" className="text-sm text-gray-400">
  ...
  return (
    <div className="h-full overflow-y-auto bg-background">
      ...
      <div className="p-8 max-w-4xl mx-auto space-y-6">
        <h1 className="text-4xl font-bold text-white mb-6">요약</h1>
        ...
        <div className="space-y-8">
          {fullSummary.topics.map((topic: Topic, topicIndex: number) => (
            <div key={topicIndex} className="space-y-4">
              <h2 className="text-2xl font-bold text-white">{topic.topic_title}</h2>
              ...
              <div className="space-y-3">
                {topic.slide_notes.map((note, noteIndex) => {
                  ...
                  if (boldMatch) {
                    ...
                    return (
                      <div key={noteIndex} className="text-white/90 text-base leading-relaxed">
              ...
              {topic.professor_emphasis.length > 0 && (
                <div className="mt-4 pt-4 border-t border-border">
                  <button
                    ...
                    className="w-full flex items-center justify-start text-left hover:bg-white/5 rounded-lg p-2 transition-colors group"
                  >
                    <div className="flex items-center gap-2">
                      <span className="text-sm text-gray-400">쉬운 설명</span>
                      <ChevronDown
                        className={`w-4 h-4 text-gray-500 transition-transform ${expandedTopics.has(topicIndex) ? "rotate-180" : ""}`}
                      />
                    </div>
                  </button>
                  {expandedTopics.has(topicIndex) && (
                    <div className="mt-3 space-y-3 pl-7">
                      {topic.professor_emphasis.map((emphasis, empIndex) => (
                        <div key={empIndex} className="text-gray-400 text-sm leading-relaxed">
                          {emphasis.point && <p className="mb-1 text-white/90">{emphasis.point}</p>}
                          {emphasis.context && <p className="text-gray-500">{emphasis.context}</p>}
                        </div>
                      ))}
                    </div>
                  )}
        ...
        {fullSummary.final_exam_takeaways.length > 0 && (
          <div className="mt-8 pt-8 border-t border-border">
            <h2 className="text-2xl font-bold text-white mb-4">시험 대비 핵심 포인트</h2>
            <div className="space-y-3">
              {fullSummary.final_exam_takeaways.map((takeaway, index) => (
                <div key={index} className="text-white/90 text-base leading-relaxed">
                  {takeaway}
                </div>
              ))}
            </div>
        ...
        <div className="text-xs text-gray-500 text-center pt-4 border-t border-border mt-8">
          <p>
